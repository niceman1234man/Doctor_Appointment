...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        //SELECT `FirstName`, `LastName`, `NIC`, `phone_number`, `email`, `date_of_birth`, `Address`, `password`, `id` FROM `patient` WHERE 1
        $sql = "UPDATE `patient` SET `FirstName`=?, `LastName`=?, `NIC`=?, `email`=?, `date_of_birth`=?, `Address`=? ,'password=?'WHERE `id`=?";
